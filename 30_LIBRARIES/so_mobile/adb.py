...
def Get_First_Connected_Device():
  ...
  if len(devices.GetSerialNumbers())>0:
    print devices
    device = devices[1]
    desired_device={'udid':device.serial,'deviceName':device.serial,'platformName':'ANDROID'}
    return desired_device
  else:
    print devices
    raise AssertionError("No Connected Device Found")
...
def Get_Device_Language():
  ...
  if len(devices.GetSerialNumbers())>0:
    print devices
    device = devices[1]
    language=device.ShellCommandOutput("getprop persist.sys.language")
    country=device.ShellCommandOutput("getprop persist.sys.country")
    language_settings={'language':language,'country':country}
